  ...
  User.findOne(
    ...
    (err, user) => {
      ...
      if (user) {
        ...
        console.log(
          `X : tentative de se connecter avec ${req.body.email} et ${
            req.body.password
          }`
        )
      }
...
authRoute.post('/register', (req, res) => {
  ...
  User.findOne({ email: req.body.email }, (err, user) => {
    ...
    if (user) {
      ...
      res.status(400).json({ status: 'exists', data: req.body.email })
    ...
    // si l'user n'est pas deja inscrit dans la collection users
      ...
      newUser.save((err, user) => {
        ...
        if (user) {
          ...
          res.status(200).json({ status: 'saved', data: newUser.email })
